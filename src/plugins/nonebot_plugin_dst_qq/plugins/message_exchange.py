...
class MessageExchangeManager:
    ...
    async def start_sync(self):
        ...
        # 初始化消息哈希集合
        await self._initialize_message_hashes()
...
def is_not_command(event: PrivateMessageEvent) -> bool:
    """检查是否为非命令消息"""
    message_content = event.get_plaintext().strip()
    command_prefixes = ["/", "消息互通", "关闭互通", "互通状态", "最新消息", "菜单", "世界", "房间", "系统", "玩家", "直连", "管理命令", "查看备份", "执行命令", "回滚世界", "踢出玩家", "封禁玩家", "解封玩家"]
    
    # 如果消息以任何命令前缀开头，返回False（不是非命令消息）
...
# 组合规则：私聊消息 且 非命令消息
def is_private_non_command(event: PrivateMessageEvent) -> bool:
    return is_private_message(event) and is_not_command(event)

private_message_handler = on_message(rule=is_private_non_command, priority=999, block=True)
...
async def handle_private_message(bot: Bot, event: PrivateMessageEvent, state: T_State):
    ...
    # 由于规则已经过滤了命令消息，这里只需要检查用户是否开启了消息互通
    if user_id not in message_manager.active_users:
